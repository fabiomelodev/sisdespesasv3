<x-filament-widgets::widget>
    <x-filament::section heading="Limite dos Cartões" icon="heroicon-o-credit-card" icon-color="danger">
        @php $alerts = $this->getAlerts(); @endphp

        @if (empty($alerts))
            <p class="fi-fo-field-wrp-helper-text text-sm text-gray-500 dark:text-gray-400">
                Nenhum cartão perto do limite.
            </p>
        @else
            <ul class="space-y-3">
                    ...
                    <li class="flex items-center justify-between gap-3">
                        <div class="min-w-0">
                            ...
                            <p class="text-xs text-gray-500 dark:text-gray-400">
                                {{ \App\Helpers\FormatCurrency::getFormatCurrency($alert['used']) }}
                                /
